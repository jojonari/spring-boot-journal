...
- 메시징
    * 메시징은 하나 또는 그 이상의 엔티티 간에 정보를 주고받는 방법입니다 
    * 메시징이 결합도를 느슨하게 만드는 분산 커뮤니케이션을 실현한다는 점입니다.
        + 즉, 송신자가 어느 메시지를 어떻게 발행하건 수신자는 송신자에게 알리지 않고 소비하는 구조입니다.
- 스프링 부트와 JMS
    * JMS는 자바에서만 쓸 수 있는 기술이라서 일종의 메시지 브리지를 만들어 다른 프로그래밍 언어와 결합해야 합니다.
    * 생산기가 큐에 메시지를 보내면 그때마다 큐를 리스닝하고 있던 소비기는 메시지를 꺼내 비지니스 로직에 따라 처리합니다.
- 스프링 부트와 래빗MQ
    * AMQP는 MOM의 공개 표준애플리케이션 레이어입니다.
    * 래빗MQ는 AMQP 프로토콜을 구현한 메시지 큐입니다.
    * 설치(MAC버전)
        + `$brew upgrade`
        + `$brew install rabbitmq`
    * JMS와 달리 AMQP는 익스체인지(exchange), 큐(Queue), 바인딩(binding) 세 가지 중요 컴포넌트로 구성됩니다.
    * 익스체인지는 발행자로부터 수신한 메시지를 큐 또는 다른 익스체인지로 보내는 라우터 역할을 합니다.
        + 이때 익스체인지는 익스체인지 타입과 바인딩이라는 몇 가지 규칙을 따르는 라우팅 알고리즘에 의해 움직입니다.
    * AMQP 프로토콜의 익스체인지 타입은 기본, 직접, 팬아웃, 토픽, 헤더 등 다섯 가지가 있습니다.
    * 핵심은 라우팅 키와 함께 메시지를 익스체인지로 보내면 익스체인지 타입별로 메시지를 큐로 보낸다는 것입니다.
        + 라우팅 키가 맞지 않으면 배달하지 않습니다.
    * 기본 익스체인지는 생성한 큐에 모두 자동 바인딩하며, 직접 익스체인지는 라우팅 키와 큐를 1대 1 바인딩 합니다. 토픽 익스체인지는 직접 익스체인지와 비슷하나, 라우팅 키와 와일드카드를 추가해서 바인딩할 수 있고, 헤더 익스체인지는 메시지 헤더에 따라 바인딩하는 기능을 더했습니다. 팬아웃 익스체인지는 메시지를 바인딩한 큐전체에 복사합니다. 말하자면 메시지를 방송하는 셈이지요.
    * `$rabbitmq-server` : 래빗QM실행
    * http://localhost:15672 : 래빗QM 관리페이지 접속
    * 스프링 부트 AMQP : http://projects.spring.io/spring-amqp
- 스프링 부트와 레디스
    * 레디스는 C언어로 개발된 키-값 저장형 NoSql DB 입니다.
